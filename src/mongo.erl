...
do (WriteMode, ReadMode, Connection, Database, Action) -> 
    ?DEBUG("~p:do ~p WriteMode = ~p, ReadMode = ~p, Connection = ~p, Database = ~p, Action = ~p", [?MODULE, ?LINE, WriteMode, ReadMode, Connection, Database, Action]),
    case connection_mode (ReadMode, Connection) of
...
command (Command) ->
        ?DEBUG("~p:command ~p Command = ~p", [?MODULE, ?LINE, Command]),
	Context = get (mongo_action_context),
