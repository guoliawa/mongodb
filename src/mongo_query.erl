...
find_one (DbConn, Query) ->
        ?DEBUG("~p:query ~p_DbConn =~p  Query = ~p", [?MODULE, ?LINE, DbConn, Query]),
...
find (DbConn, Query) ->
        %Query = Query1#query{tailablecursor=true},
        %io:format("~p:find ~p Query = ~p~n", [?MODULE, ?LINE, Query]),
...
query_reply (#reply {
	...
	cursorid = Cid, startingfrom = _, documents = Docs }) ->
                ?DEBUG("~p:query_reply ~p Docs = ~p", [?MODULE, ?LINE, Docs]),
		case QueryError of
			false -> {Cid, Docs};
